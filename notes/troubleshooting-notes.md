# Troubleshooting Notes — Data Collection Before USB Activity
...
The main troubleshooting lessons from this lab are:

1. Exclude generated output files when recursively hashing a directory.
2. Do not treat every Sysmon Event ID 11 event as suspicious.
3. Correlate Event ID 11 with Event ID 1 when process attribution is required.
4. Use hashes to validate file content.
5. Treat archive hashes separately from individual file hashes.
6. Normalize timestamps before timeline correlation.
7. Follow the evidence rather than assuming USB transfer or exfiltration.
